...
import campus.domain.*;
import campus.eventing.*;
import campus.infra.DatabaseConnection;
import campus.validation.*;
...
public class ReservationService {
    ...
    public ReservationService(DatabaseConnection db, EventBus eventBus, SchedulingService scheduling, FacultyPolicy policy) {
        ...
        this.policy = policy;
        this.validators = new FacultyPolicyValidator(policy).then(new ScheduleConflictValidator(scheduling));
    }

    public ValidationResult submit(ReservationRequest req, User actor) {
        ValidationResult vr = validators.validate(req, actor);
        if (!vr.ok) return vr;
